...
import com.rui.economystore.api.network.handler.server.BuyItemInStoreHandler;
...
public class ItemStoreScreen extends AbstractStoreScreen<ItemStoreContainer> {
    public static final TextComponent DISCOUNT_PRICE = new TranslationTextComponent(LibMisc.MOD_ID+".container.discount_price", DataHelper.getItemStoreMultiple());
    private static final DecimalFormat df = new DecimalFormat("#.00");
    public ItemStoreScreen(ItemStoreContainer p_i51105_1_, PlayerInventory p_i51105_2_, ITextComponent p_i51105_3_) {
        super(p_i51105_1_, p_i51105_2_, p_i51105_3_, ItemStoreItem.itemMap);
        this.shopItem = -1;
    }
    ...
    protected void init() {
        ...
        int k = j + 16 + 2;
        for (int l = 0; l < 7; ++l) {
            this.itemButtons[l] = this.addButton(new AbstractStoreScreen.ItemButton(i + 5, k, l, (p_214132_1_) -> {
        ...
        this.addButton(new Button(i + 200, j + 35, 28, 20, new TranslationTextComponent(LibMisc.MOD_ID + ".container.buy"), (press) -> {
//            this.menu.sellItemAndGetMoney(player);
            PacketHandler.sendToServer(new BuyItemInStoreHandler(this.shopItem));
        }));
        this.addButton(new Button(i + 231, j + 35, 28, 20, new TranslationTextComponent(LibMisc.MOD_ID + ".container.clear"), (press) -> {
            this.shopItem = -1;
        }));
    ...
    public void render(MatrixStack p_230430_1_, int p_230430_2_, int p_230430_3_, float p_230430_4_) {
        ...
        if (!itemMap.isEmpty()) {
            ...
            this.font.draw(p_230430_1_, DISCOUNT_PRICE,i+110,j+40, 4210752);
            ...
            if(shopItem>=0) {
                ...
                this.itemRenderer.renderAndDecorateFakeItem(item.getDefaultInstance(),i+162,j+37);
                this.itemRenderer.renderGuiItemDecorations(this.font, item.getDefaultInstance(), 162, 37);
                float amount = itemMap.get(item)*DataHelper.getItemStoreMultiple();
                String pay_string = df.format(amount);
                this.font.draw(p_230430_1_, new TranslationTextComponent(LibMisc.MOD_ID+".container.pay", pay_string),i+140,j+62, 4210752);
            }
